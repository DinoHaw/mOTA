...
### 三、 bootloader 架构
#### （一）软件架构
![bootloader软件架构](image/bootloader软件架构.png)
...
![bootloader的程序设计思路（YModem）](image/bootloader的程序设计思路（YModem）.png)
...
1.  最小占用（单分区）
![STM32F1_单分区](image/screenshot/STM32F1_单分区.png)
```
RAM:   8.7 KB
flash: 8.3 KB
```
...
1.  一般占用（单分区 + 解密组件）
![STM32F1_单分区_解密组件](image/screenshot/STM32F1_单分区_解密组件.png)
```
RAM:   8.9 KB
flash: 9.5 KB
```
...
1.  一般占用（三分区）
![STM32F1_三分区](image/screenshot/STM32F1_三分区.png)
```
RAM:   8.7 KB
flash: 9.4 KB
```
...
1.  一般占用（三分区 + 解密组件）
![STM32F1_三分区_解密组件](image/screenshot/STM32F1_三分区_解密组件.png)
```
RAM:    9.0 KB
flash: 10.8 KB
```
...
![STM32L4_三分区_解密组件_QSPI](image/screenshot/STM32L4_三分区_解密组件_QSPI.png)
```
RAM:   10.2 KB
flash: 25.5 KB
```
...
![update_flag的定义](image/screenshot/update_flag的定义.png)
![IDE配置RAM](image/screenshot/IDE配置RAM.png)  
...
![AC6的配置](image/screenshot/AC6的配置.png)
...
![bootloader的map](image/screenshot/bootloader的map.png)  
...
![设置中断向量表](image/screenshot/设置中断向量表.png)
![IDE设置ROM地址](image/screenshot/IDE设置ROM地址.png)  
